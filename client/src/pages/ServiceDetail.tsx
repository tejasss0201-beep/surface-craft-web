...
const details: Record<ServiceKind, {
  ...
  trash: {
    eyebrow: "Service detail / 01",
    title: "Trash bins, reset.",
    ...
    imageAlt: "Close texture of a cleaned exterior surface",
    icon: ShieldCheck,
  ...
  concrete: {
    eyebrow: "Service detail / 02",
    title: "Concrete, brought forward.",
  ...
  return (
    <main className="detail-page detail-page--service">
      <header className="detail-nav">
        <a href="/" className="detail-brand"><img src={ASSETS.logo} alt="" /><span>Surface<br /><i>Craft</i></span></a>
      ...
      <section className="detail-body detail-body--service">
        <div className="detail-body-aside"><span>Scope notes</span><i /><span>{kind === "trash" ? "01 — 03" : "01 — 04"}</span></div>
        <div className="detail-copy"><p className="detail-lede">What to expect from this part of the work.</p>{detail.sections.map((section) => <div className="detail-section-note" key={section.title}><h2>{section.title}</h2><p>{section.copy}</p></div>)}<ul className="detail-includes">{detail.includes.map((item) => <li key={item}><Check size={16} /><span>{item}</span></li>)}</ul></div>
      </section>
